...
r.post("/templates", async (req, res, next) => {
  try {
    ...
    const sc = validateSenderId(data.sender_id);
    if (!sc.ok) return res.status(400).json({ message: sc.message });
    ...
    const { rows } = await pool.query(
      ...
      [req.user.sub, data.name, sc.value, data.message]
    );
    res.json(rows[0]);
  } catch (e) { next(e); }
});
  ...
  try {
    ...
    const { rows } = await pool.query(
      `UPDATE sms_templates SET name=$1, sender_id=$2, message=$3, updated_at=now()
        ...
        RETURNING id, name, sender_id, message, created_at, updated_at`,
      [data.name, sc.value, data.message, req.params.id, req.user.sub]
    );
